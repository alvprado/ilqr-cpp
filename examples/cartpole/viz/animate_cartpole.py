...
def build_axes(ax, t, cart_x, theta, length, title=None, show_clock=True):
    ...
    (trace,) = ax.plot([], [], "-", color=_common.ACCENT_RED, alpha=_common.TRACE_ALPHA, lw=1,
                       zorder=1)
    time_text = ax.text(0.02, 0.94, "", transform=ax.transAxes) if show_clock else None
    ...
    artists = [cart, rod, pivot, trace] + ([time_text] if time_text is not None else [])
...
def main():
    ...
    artists, update = build_axes(ax, t, cart_x, theta, args.length,
                                 title="iLQR cart-pole swing-up")
